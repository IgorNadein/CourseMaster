...
from .ajax_views import (CourseBuilderView, CoursePublishAjaxView,
                         CourseUnpublishAjaxView, CourseUpdateAjaxView,
                         LessonCreateAjaxView, LessonDeleteAjaxView,
                         LessonGetAjaxView, LessonUpdateAjaxView,
                         SectionCreateAjaxView, SectionDeleteAjaxView,
                         SectionUpdateAjaxView, StepCheckAnswerView,
                         StepCompleteView, StepCreateAjaxView,
                         StepDeleteAjaxView, StepDuplicateAjaxView,
                         StepGetAjaxView, StepListAjaxView,
                         StepReorderAjaxView, StepUpdateAjaxView)
from .forms import (CheckoutForm, CourseForm, CourseMediaEditForm,
                    CourseMediaUploadForm, CoursePublishForm,
                    LessonCommentForm, LessonForm, PromoCodeForm,
                    RefundRequestForm, ReviewForm, SectionForm,
                    StripePaymentForm)
from .models import (Category, Certificate, Course, CourseMedia, Enrollment,
                     Lesson, LessonComment, LessonProgress, Payment,
                     PaymentMethod, PromoCode, Purchase, Refund, Review,
                     Section, Step, StepProgress)
...
class CourseListView(ListView):
    ...
    def get_queryset(self):
        ...
        valid_sorts = ['-created_at', 'price', '-price',
                       '-average_rating', '-students_count']
...
class CourseDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['reviews'] = course.reviews.filter(
            is_approved=True).select_related('student')[:10]
        context['reviews_count'] = course.reviews.filter(
...
class MyCoursesView(LoginRequiredMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['completed_courses'] = enrollments.filter(
        ...
        for enrollment in context['enrollments']:
            ...
            completed_lesson_ids = set(
                enrollment.lesson_progress.filter(
                    completed=True).values_list('lesson_id', flat=True)
            )
...
class LessonView(LoginRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['previous_lesson'] = lesson_list[current_index -
                                                 1] if current_index > 0 else None
        context['next_lesson'] = lesson_list[current_index +
                                             1] if current_index < len(lesson_list) - 1 else None
        ...
        if current_step:
            context['previous_step'] = step_list[current_step_index -
                                                 1] if current_step_index > 0 else None
            context['next_step'] = step_list[current_step_index +
                                             1] if current_step_index < len(step_list) - 1 else None
        ...
        if enrollment and steps.exists():
            ...
            if steps.count() > 0:
                context['steps_progress_percent'] = round(
                    (completed_steps / steps.count()) * 100)
            else:
                context['steps_progress_percent'] = 0

            # Прогресс текущего шага
            if current_step:
                context['current_step_progress'] = step_progress_dict.get(
                    current_step.id)
        ...
        context['comments_count'] = lesson.comments.filter(
            is_approved=True).count()
        context['comment_form'] = LessonCommentForm()

        # Проверка: можно ли оставлять комментарии
        is_enrolled = Enrollment.objects.filter(
            student=self.request.user, course=course).exists()
        is_instructor = course.instructor == self.request.user
...
class LessonCompleteView(LoginRequiredMixin, View):
    ...
    def post(self, request, lesson_id):
        ...
        if not lesson_progress.completed:
            ...
            messages.success(
                request, f'Урок "{lesson.title}" отмечен как пройденный.')
...
class InstructorCoursesView(LoginRequiredMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['published_courses'] = courses.filter(
...
class ReviewCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        if not Enrollment.objects.filter(student=self.request.user, course=self.course).exists():
            messages.error(
                self.request, 'Вы должны быть записаны на курс, чтобы оставить отзыв.')
            return redirect('course_detail', slug=self.course.slug)

        # Проверка: уже есть отзыв?
        existing = Review.objects.filter(
            student=self.request.user, course=self.course).first()
        if existing:
            messages.info(
                self.request, 'Вы уже оставили отзыв. Вы можете его отредактировать.')
...
class LessonCommentCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        if not (is_enrolled or is_instructor):
            messages.error(
                self.request, 'Вы должны быть записаны на курс, чтобы оставлять комментарии.')
        ...
        if reply_to_id:
            form.instance.reply_to = get_object_or_404(
                LessonComment, id=reply_to_id)
...
class CourseCheckoutView(LoginRequiredMixin, View):
    ...
    def get(self, request, slug):
        ...
        if Enrollment.objects.filter(student=request.user, course=course).exists():
            messages.info(
                request, f'Вы уже записаны на курс "{course.title}".')
            return redirect('course_detail', slug=course.slug)
                ...
                try:
                    ...
                    if promo_code.is_valid():
                        ...
                        if promo_code.applicable_courses.exists() and course not in promo_code.applicable_courses.all():
                            messages.error(
                                request, 'Этот промокод не применим к данному курсу.')
                            promo_code = None
                        else:
                            final_price = promo_code.apply_discount(
                        ...
                        messages.error(
                            request, 'Промокод неактивен или истек.')
                        promo_code = None
...
class StripePaymentView(LoginRequiredMixin, View):
    ...
    def get(self, request, purchase_id):
        purchase = get_object_or_404(
            Purchase, id=purchase_id, student=request.user, status='pending')

        context = {
            'purchase': purchase,
            'course': purchase.course,
            # Stripe требует сумму в центах
            'amount': int(purchase.total_amount * 100),
            'stripe_public_key': settings.STRIPE_PUBLIC_KEY if hasattr(settings, 'STRIPE_PUBLIC_KEY') else '',
        }
    ...
    def post(self, request, purchase_id):
        ...
        try:
            import stripe
            stripe.api_key = settings.STRIPE_SECRET_KEY if hasattr(
            ...
            messages.success(
                request, f'✓ Платеж успешно обработан! Вы записаны на курс "{purchase.course.title}".')
...
class PaymentSuccessView(LoginRequiredMixin, View):
    ...
    def get(self, request, purchase_id):
        purchase = get_object_or_404(
            Purchase, id=purchase_id, student=request.user, status='completed')
...
class PaymentFailedView(LoginRequiredMixin, View):
    ...
    def get(self, request, purchase_id):
        purchase = get_object_or_404(
            Purchase, id=purchase_id, student=request.user, status='failed')
...
class PurchaseHistoryView(LoginRequiredMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['total_spent'] = sum(
            p.total_amount for p in self.get_queryset() if p.status == 'completed')
        context['completed_purchases'] = self.get_queryset().filter(
...
class RefundRequestView(LoginRequiredMixin, CreateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        self.purchase = get_object_or_404(Purchase, id=kwargs.get(
            'purchase_id'), student=request.user, status='completed')

        # Проверка - уже ли есть запрос на возврат
        if Refund.objects.filter(purchase=self.purchase, status__in=['pending', 'approved']).exists():
            messages.error(
                request, 'Вы уже отправили запрос на возврат для этой покупки.')
    ...
    def form_valid(self, form):
        ...
        messages.success(
            request, 'Ваш запрос на возврат отправлен. Мы свяжемся с вами в течение 3-5 дней.')
        return redirect('purchase_history')
...
class MediaLibraryView(LoginRequiredMixin, UserPassesTestMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['documents_count'] = all_media.filter(
            media_type='document').count()
        context['total_size'] = sum(m.file_size for m in all_media)
        context['total_size_display'] = self._format_size(
            context['total_size'])
...
class MediaUploadView(LoginRequiredMixin, UserPassesTestMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        messages.success(
            self.request, f'Файл "{form.instance.original_filename}" успешно загружен!')
        return super().form_valid(form)
